...
It is important that the outcome is not truly random so that if planet 1652 contains angry blue humanoids, the next time it is checked they are still living there and haven’t been replaced by docile red lizards!
"""
...
def seed(x):
    ...
    for x in range(3):
        v = random.random()
        p = [sigmoid(1), sigmoid(2), sigmoid(3)]
        p1 = [1, 2, 3]
        g = (min(p, key=lambda x: abs(x - reverse_sigmoid(v))))
        g = (min(p1, key=lambda x: abs(x - reverse_sigmoid(g))))
        po.append(g)
